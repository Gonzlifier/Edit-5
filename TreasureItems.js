...
function ADeadDove(player, DeadDove) {
    const explode = this.sound.add('pickup');
    explode.play();
    DeadDove.disableBody(true, true);
    SpectralActive = true;
    FlyingActive = true;
}
function TheHolyGrail(player, HolyGrail) {
    const explode = this.sound.add('pickup');
    explode.play();
    HolyGrail.disableBody(true, true);
    FlyingActive = true;
    MaxHp += 1;
    currentHP += 1;
    HPText.setText('Current Hp: ' + currentHP)
...
function Body(player, TheBody) {
    const explode = this.sound.add('pickup');
    explode.play();
    TheBody.disableBody(true, true);
    MaxHp += 3;
    currentHP += 3;
    HPText.setText('Current Hp: ' + currentHP)
    MaxHpText.setText('Max Hp: ' + MaxHp)
}
function CeremonialRobes(player, Robes) {
    const explode = this.sound.add('pickup');
    explode.play();
    Robes.disableBody(true, true);
    currentSin += 3;
    SinText.setText('Hearts of Sin: ' + currentSin);
    BulletDMG++;
}
function SadBomb(player, Sbomb) {
    const explode = this.sound.add('pickup');
    explode.play();
    Sbomb.disableBody(true, true);
    currentBombs += 5;
    BombText.setText('Bombs: ' + currentBombs)
    SadBombActive = true;
}