...
function SADBOMB() {
    var sX = Math.random() * 780;
    var sY = Math.random() * 580;
    var Sbombs = Sbomb.create(sX, sY, 'SadBomb').setTint(0x0000ff)
    Sbombs.setCollideWorldBounds(true);
}
function BLANKET() {
    var sX = Math.random() * 780;
    var sY = Math.random() * 580;
...
function DevilAngelRoom(player, SpecialRoom) {
    if (DevilRoomChance == true) {
        var noomber = Math.round(Math.random() * 3);
        if (noomber == 0) {
            ...
            Robes.create(400, 300, 'Robes').setTint(0x000000);
        }
        ...
        if (number == 0) {
            ...
            DeadDove.create(400, 300, 'deadDove').setTint(0xFFFFFF);
        } else if (number == 3) {
            HolyGrail.create(400, 300, 'holyGrail').setAngle(-90);
        } else if (number == 4) {
            TheBody.create(400, 300, 'TheBody').setTint(0x0000ff);
        }
    }
}